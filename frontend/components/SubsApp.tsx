...
const labels = {
  ru: {
    ...
    fixed: 'Фиксированная', variable: 'Переменная', custom: 'Своя сумма', free: 'Бесплатная', unknown: 'Неизвестна', group_child: 'Входит в группу', scenario: 'Сценарий', scenarioSaving: 'Экономия', checked: 'Проверено', hide: 'Скрыть', undo: 'Отменить изменение', close: 'Закрыть', empty: 'Создайте первую подписку, чтобы увидеть прогноз и календарные события.'
  },
  en: {
    ...
    fixed: 'Fixed', variable: 'Variable', custom: 'Custom', free: 'Free', unknown: 'Unknown', group_child: 'Group child', scenario: 'Scenario', scenarioSaving: 'Saving', checked: 'Checked', hide: 'Hide', undo: 'Undo change', close: 'Close', empty: 'Create your first subscription to see forecast and calendar events.'
  }
};
...
const categoryLabels: Record<Lang, Record<string, string>> = {
  ru: { ai: 'AI', media: 'Медиа', cloud: 'Облако', mobile: 'Связь', software: 'ПО', finance: 'Финансы', education: 'Образование', gaming: 'Игры', business: 'Бизнес', utilities: 'ЖКХ', electricity: 'Электричество', water: 'Вода', heating: 'Отопление', gas: 'Газ', transport: 'Транспорт', other: 'Другое' },
  en: { ai: 'AI', media: 'Media', cloud: 'Cloud', mobile: 'Mobile', software: 'Software', finance: 'Finance', education: 'Education', gaming: 'Gaming', business: 'Business', utilities: 'Utilities', electricity: 'Electricity', water: 'Water', heating: 'Heating', gas: 'Gas', transport: 'Transport', other: 'Other' }
};

const actionLabels: Record<Lang, Record<string, string>> = {
  ru: { subscription_created: 'Создание подписки', subscription_updated: 'Обновление подписки', subscription_deleted: 'Удаление подписки', group_created: 'Создание группы', group_updated: 'Обновление группы', group_deleted: 'Удаление группы', payment_recorded: 'Оплата', moved_to_group: 'Добавление в группу', removed_from_group: 'Удаление из группы' },
  en: { subscription_created: 'Subscription created', subscription_updated: 'Subscription updated', subscription_deleted: 'Subscription deleted', group_created: 'Group created', group_updated: 'Group updated', group_deleted: 'Group deleted', payment_recorded: 'Payment recorded', moved_to_group: 'Moved to group', removed_from_group: 'Removed from group' }
};

function categoryLabel(category: string | null | undefined, lang: Lang) {
  return category ? (categoryLabels[lang][category] || category) : '—';
}
...
function formatDateLong(value?: string | null, lang: Lang = 'ru') {
  if (!value) return '—';
...
function formatMonthName(month: string, lang: Lang) {
  const [year, monthNumber] = month.split('-');
  const date = new Date(Number(year), Number(monthNumber) - 1, 1);
  return new Intl.DateTimeFormat(lang === 'ru' ? 'ru-RU' : 'en-US', { month: 'long' }).format(date);
}

function actionLabel(action: string, lang: Lang) {
  return actionLabels[lang][action] || action;
}
...
function makeSubForm(initial?: Subscription): SubForm {
  return {
    ...
    renewal_date: displayDateInput(initial?.renewal_date),
    trial_end_date: displayDateInput(initial?.trial_end_date),
...
export default function SubsApp({ initialTab }: { initialTab: Tab }) {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {tab === 'overview' && <Overview dashboard={dashboard} forecast={forecast} monthly={monthly} subs={upcoming} recommendations={recommendations} t={t} lang={lang} />}
      ...
      {tab === 'history' && <History items={audit} t={t} lang={lang} />}
    </main>
...
function Overview({ dashboard, forecast, monthly, subs, recommendations, t, lang }: { dashboard: DashboardSummary | null; forecast: ForecastSummary | null; monthly: MonthlyPoint[]; subs: Subscription[]; recommendations: Recommendation[]; t: Labels; lang: Lang }) {
  return <section className="stack">
    ...
    <div className="metric-grid">
      ...
      <Metric label={t.year} value={formatMoney(forecast?.yearly_total_minor)} muted={forecast?.next_payment ? `${formatDateLong(forecast.next_payment.date, lang)} · ${formatMoney(forecast.next_payment.amount_minor)}` : undefined} />
    </div>
    <ForecastChart monthly={monthly} t={t} compact />
    <div className="two-col"><Panel title={t.upcoming}>{subs.length ? subs.map(s => <Row key={s.id} title={s.name} meta={`${formatDateLong(s.renewal_date, lang)} · ${formatMoney(s.amount_minor, s.is_estimated)}`} />) : <Empty text={t.empty} />}</Panel><Panel title={t.attention}>{recommendations.slice(0, 5).length ? recommendations.slice(0, 5).map(r => <Row key={r.id} title={r.title} meta={r.explanation} />) : <Empty text={t.noData} />}</Panel></div>
  </section>;
}
...
function ForecastView({ monthly, forecast, subs, groups, t }: { monthly: MonthlyPoint[]; forecast: ForecastSummary | null; subs: Subscription[]; groups: Group[]; t: Labels }) {
  ...
  return <section className="stack"><div className="metric-grid"><Metric label={t.month} value={formatMoney(forecast?.monthly_total_minor)} /><Metric label={t.year} value={formatMoney(forecast?.yearly_total_minor)} /><Metric label={t.remaining} value={formatMoney(Math.max((forecast?.yearly_total_minor || 0) - saving, 0))} /><Metric label={t.scenarioSaving || 'Экономия'} value={formatMoney(saving)} /></div><ForecastChart monthly={monthly} t={t} /><Panel title={t.scenario || 'Сценарий'}>{candidates.map(item => <label className="row" key={item.id}><span><input className="compact-check" type="checkbox" checked={selected.has(item.id)} onChange={(e) => { const next = new Set(selected); if (e.target.checked) next.add(item.id); else next.delete(item.id); setSelected(next); }} /> {item.name}</span><strong>{formatMoney(item.yearly_minor, item.is_estimated)}</strong></label>)}</Panel></section>;
}

function ForecastChart({ monthly, t, compact = false }: { monthly: MonthlyPoint[]; t: Labels; compact?: boolean }) { const max = Math.max(...monthly.map(m => m.amount_minor), 1); return <div className="panel"><h2>{t.forecast12}</h2><div className={compact ? 'chart compact' : 'chart'}>{monthly.map(m => <div className="bar-wrap" key={m.month}><div className="bar" style={{ height: `${Math.max(8, (m.amount_minor / max) * (compact ? 140 : 190))}px` }} /><span>{formatMonthName(m.month, (t.addSub === 'Добавить подписку' ? 'ru' : 'en') as Lang)}</span><small>{formatMoney(m.amount_minor)}{m.estimated_minor ? ` · ${formatMoney(m.estimated_minor, true)}` : ''}</small></div>)}</div>{!monthly.length && <Empty text={t.noData} />}</div>; }

function Recommendations({ items, t, onRun, onAction }: { items: Recommendation[]; t: Labels; onRun: () => void; onAction: (id: string, action: 'dismiss' | 'mark-checked') => void }) { return <div className="stack"><button className="primary fit" onClick={onRun}>{t.runAi}</button><div className="grid-list">{items.map(item => <div className="panel" key={item.id}><Badge tone={badgeClass(item.severity)}>{item.severity}</Badge><h2>{item.title}</h2><p>{item.explanation}</p><div className="button-row"><small className="muted">confidence {Math.round(Number(item.confidence) * 100)}%</small><button className="small secondary" onClick={() => onAction(item.id, 'mark-checked')}>{t.checked || 'Проверено'}</button><button className="small secondary" onClick={() => onAction(item.id, 'dismiss')}>{t.hide || 'Скрыть'}</button></div></div>)}</div>{!items.length && <Empty text={t.noData} />}</div>; }
...
function History({ items, t, lang }: { items: AuditItem[]; t: Labels; lang: Lang }) { const [selected, setSelected] = useState<AuditItem | null>(null); return <><div className="panel"><h2>{t.history}</h2>{items.length ? items.map(item => <button className="history-row" key={item.id} onClick={() => setSelected(item)}><strong>{actionLabel(item.action, lang)}</strong><span>{item.target_type} · {new Date(item.created_at).toLocaleString(lang === 'ru' ? 'ru-RU' : 'en-US')}</span></button>) : <Empty text={t.noData} />}</div>{selected && <Modal title={actionLabel(selected.action, lang)} onClose={() => setSelected(null)}><pre className="audit-json">{JSON.stringify(selected.payload, null, 2)}</pre><div className="modal-actions"><div /> <div className="right">{selected.action.includes('created') ? <button className="danger">{t.delete}</button> : <button className="secondary">{t.undo || 'Отменить изменение'}</button>}<button className="primary" onClick={() => setSelected(null)}>{t.close || 'Закрыть'}</button></div></div></Modal>}</>; }
...
function SubscriptionModal({ initial, groups, t, lang, onClose, onSaved }: { initial?: Subscription; groups: Group[]; t: Labels; lang: Lang; onClose: () => void; onSaved: () => void }) {
  ...
  async function submit() { const body = { name: form.name, service_url: form.service_url || null, account_identifier: form.account_identifier || null, category_key: form.category_key || null, status: form.status, billing_interval: form.billing_interval, amount_model: form.group_id ? 'group_child' : form.amount_model, amount_minor: form.amount_model === 'unknown' || form.group_id ? null : form.amount_minor, renewal_date: parseDisplayDate(form.renewal_date) || null, trial_end_date: parseDisplayDate(form.trial_end_date) || null, group_id: form.group_id || null, notes: form.notes || null }; await api(initial ? `/subscriptions/${initial.id}` : '/subscriptions', { method: initial ? 'PATCH' : 'POST', body: JSON.stringify(body) }); onSaved(); }
  async function remove() { if (!initial || !window.confirm(t.delete)) return; await api(`/subscriptions/${initial.id}`, { method: 'DELETE' }); onSaved(); }
  return <Modal title={title} onClose={onClose}><Field label={t.name}><input value={form.name} onChange={e => patch({ name: e.target.value })} /></Field><div className="form-grid"><Field label={t.url}><input value={form.service_url} onChange={e => patch({ service_url: e.target.value })} /></Field><Field label={t.account}><input value={form.account_identifier} placeholder={initial?.account_identifier_masked || ''} onChange={e => patch({ account_identifier: e.target.value })} /></Field></div><div className="form-grid"><Field label={t.category}><select value={form.category_key} onChange={e => patch({ category_key: e.target.value, amount_model: variableCategories.has(e.target.value) ? 'variable' : form.amount_model })}>{categories.map(c => <option value={c} key={c}>{categoryLabel(c, lang)}</option>)}</select></Field><Field label={t.status}><select value={form.status} onChange={e => patch({ status: e.target.value })}>{statuses.map(status => <option value={status} key={status}>{statusLabel(status, lang)}</option>)}</select></Field></div><div className="form-grid"><Field label={t.interval}><select value={form.billing_interval} onChange={e => patch({ billing_interval: e.target.value })}>{intervals.map(i => <option value={i} key={i}>{intervalLabels[lang][i as keyof typeof intervalLabels.ru]}</option>)}</select></Field><Field label={t.group}><select value={form.group_id} onChange={e => patch({ group_id: e.target.value, amount_model: e.target.value ? 'group_child' : 'fixed' })}><option value="">—</option>{groups.map(g => <option value={g.id} key={g.id}>{g.name}</option>)}</select></Field></div><Field label={t.type}><select value={form.amount_model} disabled={!!form.group_id} onChange={e => patch({ amount_model: e.target.value })}>{amountModels.map(m => <option value={m} key={m}>{t[m as keyof Labels] || m}</option>)}</select></Field>{!form.group_id && form.amount_model !== 'free' && form.amount_model !== 'unknown' && <div className="form-grid"><Field label={t.month}><input value={form.monthly} onChange={e => updatePrice('monthly', e.target.value)} /></Field><Field label={t.year}><input value={form.yearly} onChange={e => updatePrice('yearly', e.target.value)} /></Field></div>}<div className="form-grid"><Field label={t.renewal}><input placeholder="дд.мм.гггг" value={form.renewal_date} onChange={e => patch({ renewal_date: e.target.value })} /></Field><Field label={t.trialEnd}><input placeholder="дд.мм.гггг" value={form.trial_end_date} onChange={e => patch({ trial_end_date: e.target.value })} /></Field></div><Field label={t.notes}><textarea value={form.notes} onChange={e => patch({ notes: e.target.value })} /></Field><div className="modal-actions"><div>{initial && <button className="danger" onClick={remove}>{t.delete}</button>}</div><div className="right"><button className="secondary" onClick={onClose}>{t.cancel}</button><button className="primary" onClick={submit}>{t.save}</button></div></div></Modal>;
}
...
function GroupModal({ initial, t, lang, onClose, onSaved }: { initial?: Group; t: Labels; lang: Lang; onClose: () => void; onSaved: () => void }) {
  ...
  function updatePrice(kind: 'monthly' | 'yearly', value: string) { const minor = toMinor(value); if (kind === 'monthly') patch({ monthly: value, yearly: fromMinor(minor * 12), amount_minor: amountFromMonthly(minor, form.billing_interval) }); else patch({ yearly: value, monthly: fromMinor(Math.round(minor / 12)), amount_minor: amountFromYearly(minor, form.billing_interval) }); }
  async function submit() { const body = { name: form.name, service_url: form.service_url || null, status: form.status, billing_interval: form.billing_interval, amount_model: form.amount_model, amount_minor: form.amount_model === 'unknown' ? null : form.amount_minor, renewal_date: parseDisplayDate(form.renewal_date) || null, notes: form.notes || null }; await api(initial ? `/groups/${initial.id}` : '/groups', { method: initial ? 'PATCH' : 'POST', body: JSON.stringify(body) }); onSaved(); }
