...
def status(request: Request, current_user=Depends(get_current_user), session: Session = Depends(get_db)):
    ...
    pending += session.scalar(select(func.count(SubscriptionGroup.id)).where(SubscriptionGroup.owner_subject_id == current_user.user_id, SubscriptionGroup.calendar_sync_status.in_(["pending", "not_synced"]))) or 0
    ...
    synced += session.scalar(select(func.count(SubscriptionGroup.id)).where(SubscriptionGroup.owner_subject_id == current_user.user_id, SubscriptionGroup.calendar_sync_status == "synced")) or 0
...
def sync_now(request: Request, current_user=Depends(get_current_user), session: Session = Depends(get_db)):
    ...
    groups = session.scalars(
        select(SubscriptionGroup).where(
            ...
            SubscriptionGroup.status.in_(["active", "trial"]),
        )
    ).all()
