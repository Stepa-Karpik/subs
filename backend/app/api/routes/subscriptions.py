...
@router.get("")
def list_subscriptions(request: Request, q: str | None = None, status_filter: str | None = Query(default=None, alias="status"), interval: str | None = None, category: str | None = None, group_id: UUID | None = None, standalone: bool = False, limit: int = Query(default=100, ge=1, le=500), offset: int = Query(default=0, ge=0), current_user=Depends(get_current_user), session: Session = Depends(get_db)):
    items, total = SubscriptionService(session).list(current_user.user_id, q=q, status=status_filter, interval=interval, category=category, group_id=group_id, standalone=standalone, limit=limit, offset=offset)
